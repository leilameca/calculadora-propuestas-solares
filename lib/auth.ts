...
export async function verifySessionToken(token:string){ return z.object({userId:z.string().min(1),companyId:z.string().optional(),role:z.enum(["SUPERADMIN","COMPANY_ADMIN","SALES","ENGINEER","VIEWER"]),email:z.string()}).parse((await jwtVerify(token,secret(),{algorithms:["HS256"]})).payload); }
