...
export async function verifyEdgeSession(token:string){
  ...
  const secret=process.env.AUTH_SECRET;if(!secret&&process.env.NODE_ENV==="production")return false;
  const key=await crypto.subtle.importKey("raw",encoder.encode(secret||"development-only-secret-change-me"),{name:"HMAC",hash:"SHA-256"},false,["verify"]);
  ...
  try{const data=JSON.parse(new TextDecoder().decode(decodeBase64Url(payload))) as {exp?:number};return typeof data.exp==="number"&&Number.isFinite(data.exp)&&data.exp>Date.now()/1000}catch{return false}
}
