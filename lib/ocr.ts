...
export interface InvoiceOcrResult { customerName?:string; nic?:string; address?:string; tariff?:string; utility?:string; consumption:BilledConsumption[]; rawText:string }
export function parseElectricInvoice(text:string):InvoiceOcrResult {
  ...
  const customerName=normalized.match(/(?:CLIENTE|NOMBRE|TITULAR)\s*[:#-]?\s*([^\n]{3,80})/i)?.[1]?.trim();
  ...
  const unique=[...new Map(consumption.map((item)=>[`${item.year}-${item.month}`,item])).values()].sort((a,b)=>(a.year-b.year)||(a.month-b.month)).slice(-12);
  return {customerName,nic,address,tariff,utility,consumption:unique,rawText:text};
}
