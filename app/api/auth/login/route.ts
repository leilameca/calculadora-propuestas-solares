...
import { createSessionToken, sessionMaxAgeSeconds } from "@/lib/auth";
...
async function handlePOST(request:NextRequest){
  ...
  response.cookies.set("solar_session",token,{httpOnly:true,secure:process.env.NODE_ENV==="production",sameSite:"lax",path:"/",maxAge:sessionMaxAgeSeconds()});
  return response;
}
