...
type Proposal = {
  id:string; number:string; version:number; projectName:string; status:Status; totalUsd:string; createdAt:string;
...
export default function ProposalsPage(){
  ...
  useEffect(()=>{fetch("/api/proposals").then(r=>r.ok?r.json():[]).then(rows=>setProposals(Array.isArray(rows)?rows:[])).finally(()=>setLoading(false))},[]);
  ...
  async function changeVersion(proposal:Proposal,version:number){
    if(!Number.isInteger(version)||version<1)return;
    setBusy(`version-${proposal.id}`);setMessage("");
    try{const response=await fetch("/api/proposals",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:proposal.id,version})});const data=await response.json();if(!response.ok)throw new Error(data.error||"No se pudo cambiar la versión.");setProposals(rows=>rows.map(row=>row.id===proposal.id?{...row,version:data.version}:row));setMessage(`${proposal.number} quedó en versión ${data.version}.`)}catch(error){setMessage(error instanceof Error?error.message:"No se pudo cambiar la versión.")}finally{setBusy(null)}
  }

  async function download(proposal:Proposal,format:"pdf"|"docx"){
    setBusy(`${format}-${proposal.id}`);setMessage("");
    try{
      const response=await fetch(`/api/proposals/${format}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({proposalId:proposal.id})});
