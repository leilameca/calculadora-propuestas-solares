...
type Customer={id:string;name:string;nic?:string|null;address?:string|null;city?:string|null;utility?:string|null;tariff?:string|null};
...
function defaultQuote(total:number,panels:number,panelWatts:number,inverter:string,systemType:string,battery:string) { const rows=systemType==="On-Grid"?[["Módulos bifaciales",`${panels} módulos de ${panelWatts} W`,.38],["Inversor",inverter||"A seleccionar",.24],["Estructura galvanizada","Sistema certificado",.11],["Cableado y protecciones","DC/AC y puesta a tierra",.1],["Mano de obra e ingeniería","Instalación y puesta en marcha",.17]]:[["Módulos bifaciales",`${panels} módulos de ${panelWatts} W`,.32],["Inversor",inverter||"A seleccionar",.2],["Batería",battery||"Capacidad por seleccionar",.2],["Estructura galvanizada","Sistema certificado",.1],["Cableado y protecciones","DC/AC y puesta a tierra",.08],["Mano de obra e ingeniería","Instalación y puesta en marcha",.1]]; return rows.map(([name,description,share])=>({name:String(name),description:String(description),amountUsd:total*Number(share)})); }
