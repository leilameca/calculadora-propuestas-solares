...
type Inputs = { client:string; nic:string; address:string; city:keyof typeof HSP_BY_CITY; utility:Utility; tariff:Tariff; panelWatts:number; oversizingFactor:number; costPerWpUsd:number; exchangeRate:number; systemType:string; inverter:string; itbisEnabled:boolean; itbisRate:number };
type CompanyBrand={name:string;rnc?:string;phone?:string;email?:string;address?:string;slogan?:string;primaryColor:string;secondaryColor:string;accentColor:string;proposalValidityDays:number;logoUrl?:string;coverImages?:string[];itbisEnabled?:boolean;itbisRate?:number|string|null};
...
export function SolarCalculatorApp() {
  const [inputs, setInputs] = useState<Inputs>({ client:"",nic:"",address:"",city:"Santiago",utility:"EDENORTE",tariff:"BTS-1",panelWatts:590,oversizingFactor:1.2,costPerWpUsd:.95,exchangeRate:60,systemType:"On-Grid",inverter:"",itbisEnabled:true,itbisRate:.18 });
  ...
  async function exportDocx() {
    ...
    setExporting(true);
    try {
      const response = await fetch("/api/proposals/docx",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({ company:{...company,logoBase64:company.logoUrl,coverImageBase64:company.coverImages?.[0],backCoverImageBase64:company.coverImages?.[1]||company.coverImages?.[0]}, customer:{name:inputs.client||"Cliente de demostración",nic:inputs.nic||"N/D",address:inputs.address||inputs.city}, project:{name:"Sistema Solar Fotovoltaico",city:inputs.city,utility:inputs.utility,tariff:inputs.tariff,systemType:inputs.systemType,panelWatts:inputs.panelWatts,inverter:inputs.inverter||"Por seleccionar"},consumption:effectiveConsumption,result,quoteItems:defaultQuote(result.costUsd,result.panelCount,inputs.panelWatts,inputs.inverter)})});
