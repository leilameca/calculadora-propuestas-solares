...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(()=>{fetch("/api/me").then(r=>r.ok?r.json():null).then(data=>{if(data?.role)setRole(data.role);const brand=data?.company;if(brand?.name)setCompany(brand.name);if(brand?.primaryColor){const toRgb=(hex:string)=>{const clean=hex.replace("#","");return `${parseInt(clean.slice(0,2),16)} ${parseInt(clean.slice(2,4),16)} ${parseInt(clean.slice(4,6),16)}`};document.documentElement.style.setProperty("--brand-primary",toRgb(brand.primaryColor));document.documentElement.style.setProperty("--brand-secondary",toRgb(brand.secondaryColor));document.documentElement.style.setProperty("--brand-accent",toRgb(brand.accentColor));}}).catch(()=>undefined)},[]);
