...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-slate-50">
    <aside className={cn("fixed inset-y-0 left-0 z-40 w-64 border-r bg-slate-950 text-white transition-transform lg:translate-x-0", open ? "translate-x-0" : "-translate-x-full")}>
      ...
      <nav className="space-y-1 p-3">{nav.map(([href, label, Icon]) => <Link key={href} href={href} onClick={() => setOpen(false)} className={cn("flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-300 hover:bg-white/10 hover:text-white", pathname === href && "bg-primary text-white")}><Icon size={18} />{label}</Link>)}</nav>
      <div className="absolute inset-x-3 bottom-4 border-t border-white/10 pt-3"><button className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-slate-400 hover:bg-white/10 hover:text-white"><LogOut size={17} />Cerrar sesión</button></div>
    </aside>
    <div className="lg:pl-64">
      <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b bg-white/90 px-4 backdrop-blur lg:px-8">
        ...
        <div className="ml-auto flex items-center gap-3"><div className="text-right"><p className="text-sm font-semibold">EILEN Electric Service</p><p className="text-xs text-slate-500">Administrador</p></div><div className="grid size-9 place-items-center rounded-full bg-primary text-sm font-bold text-white">EE</div></div>
      </header>
